...
async def facebook_callback(code: str = Query(...)):
    ...
    user_id = await save_user(access_token)
    
    app_access_token = create_jwt_token(user_id, expires_delta=timedelta(minutes=15))
    app_refresh_token = create_jwt_token(user_id, expires_delta=timedelta(days=7))
